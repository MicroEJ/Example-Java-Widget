...
/**
 * Renders a #LinedToggleButton widget.
 */
public class LinedToggleButtonRenderer extends WidgetRenderer {
	...
	public int getPreferredContentWidth(Widget widget) {
		return getPictoFont().charWidth(Pictos.SWITCH_ON) + 2 * INTERNAL_MARGIN;
	}
	...
	@Override
	public Class<LinedToggleButton> getManagedType() {
	...
	public void render(GraphicsContext g, Renderable renderable) {
		...
		if (toggleButton.isSelected()) {
			...
			g.drawChar(Pictos.SWITCH_ON, width / 2, height / 2, GraphicsContext.HCENTER | GraphicsContext.VCENTER);
			g.setColor(backgroundColor);
			g.setFont(getFont());
			...
			g.drawChar(Pictos.SWITCH_OFF, width / 2, height / 2, GraphicsContext.HCENTER | GraphicsContext.VCENTER);
			g.setFont(getFont());
	 ...
	 * @param toggleButton
	 *            the toggleButton to under and overline.
	 * @param width
